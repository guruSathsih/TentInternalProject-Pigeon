		...
		*/
		...
		private function login(){
			...
			if(!empty($mobile_no) and !empty($password)){ 
					$sql = mysql_query("SELECT * FROM users WHERE mobile_no = '$mobile_no' AND password = '$password' LIMIT 1", $this->db);
		...
		*/
		...
		private function signuppigeon(){ 
			...
			if(!empty($mobile) and !empty($password)){ 
					...
					mysql_query("INSERT INTO USERS VALUES('$mobile','$name','$email','$password','$profile','$status')",$this->db); 
